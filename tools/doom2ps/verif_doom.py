...
def main(argv=None):
    ...
    #     voit -- compter les faces, comparer deux builds ou croiser sol et plafond laisse passer le
    #     cas ou les DEUX manquent au meme endroit (on voit alors a travers le niveau).
    #     Le juge est l'EMPREINTE de la feuille : une feuille du .LEV est CONVEXE, donc son contour
    #     au sol est l'enveloppe convexe des extremites de ses murs verticaux. On echantillonne tous
    #     les 4 u ; un point compte comme peint s'il tombe dans une face du MEME plan de N'IMPORTE
    #     QUELLE feuille, parce que le debord fait precisement peindre un carre par le voisin.
    #     Un plan PARALLAX (ciel) n'emet aucune face : sa feuille est exclue pour ce plan-la.
    #     MESURE 16-09 : la conversion laisse 560 u2 au sol et 464 u2 au plafond, en eclats de 16 a
    #     128 u2 par feuille -- des bords diagonaux que la grille d'echantillonnage rate, pas des
    #     trous. Un VRAI trou est d'un tout autre ordre : l'election de proprietaire essayee le 16-09
    #     en a creuse un de 3 760 u2 (9,5 % de la feuille 189, sol ET plafond a 0 %). Le seuil est
    #     donc a 256 u2 par feuille : le double du pire eclat, le quinzieme du trou.
    SEUIL_TROU, PAS = 256.0, 4.0
    ...
    for si, s_ in enumerate(S):
        for wi in range(s_["firstWall"], s_["lastWall"] + 1):
            ...
            for fi in range(w["firstFace"], w["lastFace"] + 1):
                ...
                xs = [v[0] for v in r]
                zs = [v[1] for v in r]
                for gx in range(int(math.floor(min(xs) / 64.0)), int(math.ceil(max(xs) / 64.0))):
                    for gz in range(int(math.floor(min(zs) / 64.0)), int(math.ceil(max(zs) / 64.0))):
    ...
    for si, s_ in enumerate(S):
        ...
        for p in (0, 1):
            ...
            while x < max(xs):
                ...
                while z < max(zs):
                    if dans(h, x, z):
                        g = (int(math.floor(x / 64.0)), int(math.floor(z / 64.0)))
    ...
    put(f"aucun trou dans un sol ou un plafond (> {SEUIL_TROU:.0f} u2 par feuille)", not trous,
        (f"{len(trous)} feuilles, ex. {sorted(trous, key=lambda t: -t[2])[:4]}" if trous else
         f"residu {total[0]:.0f} u2 au sol, {total[1]:.0f} u2 au plafond"))
