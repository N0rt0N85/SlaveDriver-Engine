...
ou qui descend, 6 shorts, §6) **`OT_DOOM_DOOR` 183** (porte, 6 shorts, §6), **`OT_DOOM_LIFT` 184** (ascenseur downWaitUpStay : `pb`, course < 0, canal, vitesse en 1/8 u par tic, attente en tics — 5 shorts) et **`OT_DOOM_WLINE` 185** (ligne W franchie par le centre du joueur : `x1, z1, x2, z2`, canal, drapeaux 1 = W1 — 6 shorts) — ces cinq-là ne vivent que
...
| interrupteur S | `OT_SW1` 168 | `sectorNm`, `channel`, `ox, oy, oz` | AI2.C:582-596 ; press sur tout le mur (`CFG_SWITCH_AIM` 0, AI2.C:535 ; PowerSlave : < 40 u de l'orifice) | 1 (sortie) : orifice = centre du mur |
...
| sol qui monte (5/91 W, 101/64 S au plus bas plafond voisin ; 119/128 W, 18/69 S au prochain sol ; turbo 129/130/131/132 ; plates-formes 22/95 W, 20/68 S prochain sol + flat ; +24/+32/+512 : 58/92/59/93 W, 15/66/14/67/140 S ; **escaliers** 8/100 W, 7/127 S : une marche = un objet, destination de la chaîne d'EV_BuildStairs, `doom_specials.stair_steps`) | `OT_DOOM_FLOOR` 182 | `pb`, `throw`, `channel = tag`, `speed` (1/8 u par tic : 8 = FLOORSPEED, 4 = plates-formes, 32 = turbo), `donorFace` (face dont le flat remplace le sien au départ, −1 = aucun), `damage` (hp ensuite, −1 = inchangés), **`back`** (le canal qui le renvoie à l'état du WAD, −1 = aller simple) et **`backSpeed`** — **8 shorts** | nouveau, `game/doom` : géométrie émise **en haut** (un ascenseur en haut, `doom3d.raise_floors`), descendue de `throw` au placement, montée sur `SIGNAL_SWITCH(channel)`, redescendue sur `SIGNAL_SWITCH(back)` | E1M3 ×3 (20), E1M4 ×3 (5, 18), E1M5 ×1 (22), E1M7 ×1 (22), E1M8 ×1 (91), E1M9 ×1 (20) |

**LES DEUX BOUTS (24-09).** Doom ne donne pas UN mécanisme à un secteur : il lui donne un sol, et
chaque special qui nomme son tag le déplace où SA règle dit, recalculée au déclenchement. Un secteur
visé par deux specials de sens contraires était converti avec le PREMIER, le second jeté sans un mot
— et ce second est la sortie du niveau : les deux blocs d'E1M5 (tag 1) descendent dans la fosse avec
le joueur et ce sont les dix lignes 91 du fond qui les — et le — ramènent ; sans elles la fosse est un
piège et la carte ne se finit pas. Même forme sur E1M7 (tag 3) et sur E1M8, dont la première
plate-forme (tag 2) montait sans jamais redescendre. La note qui tenait ici (« les 91 d'E1M5 et E1M7
visent un sol déjà à destination, Doom ne le bouge pas ») lisait la destination sur l'état du WAD :
elle est vraie **avant** que le sol descende, et fausse après. Les deux bouts sont donc portés par le
même objet, chacun sur son canal ; les deux sens partageant le même tag dans le WAD, le convertisseur
donne au retour un **canal neuf** (902 et au-delà, hors des tags). Un secteur qui voudrait une
TROISIÈME destination est refusé et nommé, pas converti à moitié : `verif_doom.py` contrôle les deux
(« les deux bouts », « aucun secteur à plus de deux destinations ») et `make_e1m1.py` échoue sur tout
mécanisme non émis.
