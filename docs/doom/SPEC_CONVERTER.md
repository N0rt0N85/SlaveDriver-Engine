...
| rotations | lump `XXXXFR` ou `XXXXFRFR` (`F` lettre, `R` 0..8) ; vue moteur `k` = rotation `k+1` ; lump 8 chars = deux vues, la seconde en **miroir** (`flags = 1`, `chunkx' = −chunkx − 64`) | ABI §2 ; STATIC.C:512-515 |
...
| tailles | bloc niveau < 900 000 (LEVEL.C:41), séquences < 1 Mo ; **somme résidente** niveau + palettes + tuiles + séquences (E1M1 ≈ 233 978 + 12 290 + ~580 Ko + 204 Ko + 5 816 ≈ 1,04 Mo) **≤ pool réel** = LWRAM 1 Mo (0x200000-0x300000) + (0x06100000 − `_end`) lu dans `build/doom/MAIN.map` (`_end = 0x06095c18` sur le build par défaut ⇒ 435 176 o ; UTIL.C:352-359) − marge de croissance de MAIN.BIN (plan §4.2, 80-100 Ko) — pas la constante `DEMANDE_MAX` d'`assemble.py:45` | — |
